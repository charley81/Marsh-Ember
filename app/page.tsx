import { MenuDiscovery, type MenuPreview } from "@/components/home/menu-discovery";
...
const menuLabels: Record<MenuRecord["category"], string> = {
  dinner: "Dinner",
  brunch: "Brunch",
  spirits: "Cocktails",
  wine: "Selected Wines",
...
function menuPreview(menu: MenuRecord): MenuPreview {
  const allItems = menu.sections.flatMap((section) => section.items);
  const featuredItems = allItems.filter((item) => item.featuredOnLanding);
  return {
    id: menu.category,
    label: menuLabels[menu.category],
    title: menu.title,
    intro: menu.summary,
    items: (featuredItems.length ? featuredItems : allItems).slice(0, 3),
  };
}

export default async function Home() {
  const [settings, menus] = await Promise.all([getSiteSettings(), getMenus()]);
  const menuPreviews = menus.map(menuPreview);
  if (menuPreviews.length !== 4) throw new Error("Required menu content is unavailable");
  ...
  return <>
    ...
    <section className="section"><div className="section__inner"><MenuDiscovery menus={menuPreviews} /><Actions centered><ButtonLink href="/menus">Explore the Menus</ButtonLink></Actions></div></section>
    ...
    <section className="section section--navy home-visit"><div className="section__inner"><SectionHeading eyebrow="Location & Hours" title="Join us in Charleston" intro="Dinner Sunday through Thursday until 10 PM, and Friday and Saturday until 11 PM." centered light /><FactGrid facts={[{ label: "Our Address", value: <address><a href={settings.mapUrl} target="_blank" rel="noreferrer">{settings.address}</a></address> }, { label: "Our Hours", value: <span>{settings.hours.map((row) => <span className="fact-grid__line" key={row.days}>{row.days} {row.time}</span>)}</span> }]} /><Actions centered><ButtonLink href="/visit" variant="light">Plan Your Visit</ButtonLink><ButtonLink href={settings.mapUrl} variant="light" external>Get Directions</ButtonLink></Actions></div></section>
