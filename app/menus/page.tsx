...
export default async function MenusPage() {
  ...
  const spirits = previewItems(spiritsMenu);
  const wines = previewItems(wineMenu);
  ...
  return <div className="menus-page">
    ...
    <section id="our-menus" className="section menu-pathways-section"><div className="section__inner"><SectionHeading title="Our Menus" /><div className="menu-pathways">
      <article className="menu-pathway"><h3>{dinnerMenu.title}</h3><p>{dinnerMenu.summary}</p>{dinnerMenu.service ? <p className="meta">{dinnerMenu.service}</p> : null}<ButtonLink href="/menus/dinner" variant="text">View Dinner Menu</ButtonLink></article>
      <article className="menu-pathway"><h3>{brunchMenu.title}</h3><p>{brunchMenu.summary}</p>{brunchMenu.service ? <p className="meta">{brunchMenu.service}</p> : null}<ButtonLink href="#brunch" variant="text">View Weekend Brunch</ButtonLink></article>
      <article className="menu-pathway"><h3>{spiritsMenu.title}</h3><p>{spiritsMenu.summary}</p>{spiritsMenu.service ? <p className="meta">{spiritsMenu.service}</p> : null}<ButtonLink href="#spirits" variant="text">View Cocktails &amp; Spirits</ButtonLink></article>
      <article className="menu-pathway"><h3>{wineMenu.title}</h3><p>{wineMenu.summary}</p>{wineMenu.service ? <p className="meta">{wineMenu.service}</p> : null}<ButtonLink href="#cellar" variant="text">View Wine List</ButtonLink></article>
    </div></div></section>
    ...
    <section className="section menus-drinks"><div className="section__inner card-grid card-grid--2"><div id="spirits"><SectionHeading eyebrow="Spirits" title="Cocktails shaped by the season" intro="Curated concoctions reflecting Southern herbs, dynamic amari profiles, and artisanal wood-fire smoked infusions." /><div className="menus-drinks__items">{spirits.map((item) => <MenuItem item={item} key={item.name} />)}</div></div><div id="cellar"><SectionHeading eyebrow="Cellar" title="Wines for the table" intro="Bottles selected from family estates and coastal vineyards, balanced to cut the richness of wood-fired smoke and culinary fats." /><div className="menus-drinks__items">{wines.map((item) => <MenuItem item={item} key={item.name} />)}</div></div></div></section>
